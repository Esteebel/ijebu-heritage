    ...
    <div class="container-fluid py-4">
      <div class="mb-4">
        ...
        <h1 class="h3 mt-2 mb-0">
          {{ personality ? 'Edit Figure' : 'Add New Figure' }}
        </h1>
      </div>
        ...
        <div class="col-lg-8">
          <div class="card shadow-sm">
            <div class="card-header bg-white border-bottom">
              <h5 class="card-title mb-0">Figure Information</h5>
            </div>
            <div class="card-body">
              <form @submit.prevent="submit">
                <div class="row g-4">
                  ...
                  <div class="col-md-6">
                    <label for="tribe" class="form-label">Tribe/Ethnicity</label>
                    <input
                      id="tribe"
                      v-model="form.tribe"
                      type="text"
                      class="form-control"
                      :class="{ 'is-invalid': form.errors.tribe }"
                      placeholder="e.g., Ijebu, Yoruba"
                    >
                    <div class="invalid-feedback" v-if="form.errors.tribe">
                      {{ form.errors.tribe }}
                    </div>
                  </div>

                  <div class="col-md-6">
                    <label for="contributions" class="form-label">Major Contributions</label>
                    <textarea
                      id="contributions"
                      v-model="form.contributions"
                      rows="2"
                      class="form-control"
                      :class="{ 'is-invalid': form.errors.contributions }"
                      placeholder="Key achievements and contributions"
                    />
                    <div class="invalid-feedback" v-if="form.errors.contributions">
                      {{ form.errors.contributions }}
                    </div>
                  </div>

                  <div class="col-12">
                    <label for="achievements" class="form-label">Detailed Achievements</label>
                    <textarea
                      id="achievements"
                      v-model="form.achievements"
                      rows="3"
                      class="form-control"
                      :class="{ 'is-invalid': form.errors.achievements }"
                    />
                    <div class="invalid-feedback" v-if="form.errors.achievements">
                      {{ form.errors.achievements }}
                    </div>
                  </div>
                ...
                <div class="mt-4 d-flex justify-content-end">
                  <inertia-link
                    v-if="personality"
                    :href="$route('admin.personalities.show', personality.id)"
                  ...
                  >
                    ...
                    {{ personality ? 'Update Figure' : 'Create Figure' }}
                  </button>
                </div>
              </form>
            </div>
          ...
          <div class="card shadow-sm">
            ...
            <div class="card-body">
              ...
              <div class="mb-3">
                ...
                <input type="text" class="form-control" :value="personality ? formatDate(personality.updated_at) : 'Now'" readonly>
              </div>

              <div class="mb-0">
                <label class="form-label">Created By</label>
                <input type="text" class="form-control" :value="personality ? personality.created_by : currentUser()" readonly>
              </div>
            </div>
          </div>

          <!-- Image Information Card -->
          <div v-if="personality && personality.portrait" class="card shadow-sm mt-4">
            <div class="card-header bg-white border-bottom">
              <h5 class="card-title mb-0">Current Portrait</h5>
            </div>
            <div class="card-body">
              <div class="text-center">
                <img :src="'/storage/' + personality.portrait.path" :alt="personality.portrait.filename" class="img-fluid rounded mb-3" style="max-height: 200px;">
                <div>
                  <strong>{{ personality.portrait.filename }}</strong>
                  <div class="text-muted">{{ personality.portrait.mime }} • {{ formatBytes(personality.portrait.size) }}</div>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      form: this.$inertia.form({
        ...
        biography: this.personality?.biography || '',
        birth_year: this.personality?.birth_year || null,
        death_year: this.personality?.death_year || null,
        tribe: this.personality?.tribe || '',
        contributions: this.personality?.contributions || '',
        achievements: this.personality?.achievements || '',
        is_featured: this.personality?.is_featured || false,
        media_id: this.personality?.media_id || '',
        image: null,
        imagePreview: null
      })
    }
  ...
  methods: {
    ...
    submit() {
      ...
      formData.append('birth_year', this.form.birth_year || '')
      formData.append('death_year', this.form.death_year || '')
      formData.append('tribe', this.form.tribe || '')
      formData.append('contributions', this.form.contributions || '')
      formData.append('achievements', this.form.achievements || '')
      ...
      if (this.personality) {
        this.form.post(this.$route('admin.personalities.update', this.personality.id), {
          data: formData,
          headers: {
        ...
        this.form.post(this.$route('admin.personalities.store'), {
          data: formData,
          headers: {
            'Content-Type': 'multipart/form-data',
          },
