<template>
  <AdminLayout>
    <div class="container-fluid">
      <div class="row mb-4">
        <div class="col">
          <h1>Add Oral History Archive</h1>
          <p class="text-muted">Add a new oral history archive to the collection</p>
        </div>
      </div>
          ...
          <form @submit.prevent="submit">
            ...
            <div class="d-flex gap-2">
              ...
              <button type="submit" class="btn btn-primary" :disabled="processing">
                ...
                <span v-else>Save Oral History Archive</span>
              </button>
            </div>
          </form>
        </div>
...
export default {
  name: 'OralHistoryArchivesCreate',
  layout: AdminLayout,
  props: {
    mediaItems: {
      type: Array,
  ...
  methods: {
    ...
    async submit() {
      ...
      try {
        await this.$inertia.post(this.$route('admin.oral.history.archives.store'), formData, {
          onSuccess: () => {
        ...
        this.processing = false;
        console.error('Error saving oral history archive:', error);
      }
    }
  }
}
