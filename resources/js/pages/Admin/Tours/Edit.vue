    ...
    <div class="container-fluid">
      <div class="row mb-4">
        <div class="col">
          ...
          <p class="text-muted">Update tour details</p>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        max_participants: this.tour.max_participants,
        price: this.tour.price,
        featured_image_id: this.tour.featured_image_id,
        is_published: this.tour.is_published,
        image: null,
        imagePreview: null
      },
      errors: {},
  ...
  computed: {
    selectedMedia() {
      if (!this.form.featured_image_id) return null;
      return this.mediaItems.find(media => media.id === this.form.featured_image_id);
    }
  },
  methods: {
    async loadMedia() {
    ...
    async submit() {
      ...
      try {
        await this.$inertia.post(this.$route('admin.tours.update', this.tour.id), formData, {
          onSuccess: () => {
